...
def get_dnalst(dnatxt): #run this chunk with (dnatxt) from previous function
    ...
    Returns:
      string
    """
    Tseq = dnatxt #set Tseq as dnatxt for filtering unnecessary mark that will disturb data processing
    unlsted = [".",",",";",":",'"'," ","'","@","#","*"] #unlsted contains symbols that need to be removed
    for mark in unlsted: #loop through punctuation list
      Tseq = Tseq.replace(mark,"") #replace each punctuation mark in the dnatext and save the new string in Tseq
